...
(function() {
  ...
  const STORAGE_KEYS = {
    ...
    pendingKeywordAdds: 'st_pending_keyword_adds_v1',
  ...
  window.ShopThatData = {
    ...
    addKeywordsToGraph(keywordNames, options = {}) {
      const rawNames = Array.isArray(keywordNames) ? keywordNames : [];
      const seenInput = new Set();
      const names = rawNames
        .map(name => String(name || '').replace(/\s+/g, ' ').trim())
        .filter(Boolean)
        .filter(name => {
          const key = this.normalizeKeywordName(name);
      ...
      const parentName = String(options.parentName || 'Patient Chart Keywords').trim();
      const source = String(options.source || 'patient-chart').trim();
      const now = new Date().toISOString();
      const keywords = this.getKeywords().slice();
      const connections = this.getConnections().slice();
      const keywordKeys = new Set(keywords.map(k => this.normalizeKeywordName(k.name || k.id)));
      const linkKeys = new Set(connections.map(c => {
        const a = this.normalizeKeywordName(c.source);
        const b = this.normalizeKeywordName(c.target);
        return a < b ? a + '|' + b : b + '|' + a;
      }));
      const added = [];
      const skipped = [];

      function addConnectionOnce(sourceName, targetName) {
        if (!sourceName || !targetName || sourceName === targetName) return;
        const a = window.ShopThatData.normalizeKeywordName(sourceName);
        const b = window.ShopThatData.normalizeKeywordName(targetName);
        const key = a < b ? a + '|' + b : b + '|' + a;
        if (linkKeys.has(key)) return;
        linkKeys.add(key);
        connections.push({ source: sourceName, target: targetName });
      }

      if (parentName && !keywordKeys.has(this.normalizeKeywordName(parentName))) {
        keywordKeys.add(this.normalizeKeywordName(parentName));
        keywords.push({
          id: parentName,
          name: parentName,
          value: 78,
          group: options.parentGroup || 2,
          parent: options.rootName || 'LVMH',
          isArea: true,
          source,
          uses: 0,
          cost: 0,
      ...
      names.forEach(name => {
        ...
        if (keywordKeys.has(key)) {
          skipped.push(name);
          addConnectionOnce(parentName, name);
          return;
        }
        keywordKeys.add(key);
        added.push(name);
      ...
      if (parentName) addConnectionOnce(options.rootName || 'LVMH', parentName);
      this.saveKeywords(keywords);
      this.saveConnections(connections);

      if (added.length) {
        this.setPendingKeywordAdds({
          keywords: added,
          source,
          created: now
        });
        window.dispatchEvent(new CustomEvent('kw-data-updated', {
          detail: { addedKeywords: added, source }
        }));
      }
    ...
    setPendingKeywordAdds(payload) {
      const data = payload && Array.isArray(payload.keywords)
        ? {
            keywords: payload.keywords.slice(),
            source: payload.source || 'keyword-add',
            created: payload.created || new Date().toISOString()
          }
        : null;
      if (!data || !data.keywords.length) {
        window.ShopThatStorage.remove(STORAGE_KEYS.pendingKeywordAdds);
        return;
      }
      window.ShopThatStorage.write(STORAGE_KEYS.pendingKeywordAdds, data);
      this.notifyChange('pendingKeywordAdds', data);
    },

    getPendingKeywordAdds() {
      const data = window.ShopThatStorage.readObject(STORAGE_KEYS.pendingKeywordAdds);
      return data && Array.isArray(data.keywords) ? data : null;
    },

    consumePendingKeywordAdds() {
      const data = window.ShopThatStorage.readObject(STORAGE_KEYS.pendingKeywordAdds);
      window.ShopThatStorage.remove(STORAGE_KEYS.pendingKeywordAdds);
      return data && Array.isArray(data.keywords) ? data : null;
    },
  ...
  function showKeywordSyncToast(title, keywords) {
    const names = Array.isArray(keywords) ? keywords : [];
    const existing = document.querySelector('.keyword-sync-toast');
    if (existing) existing.remove();
    const toast = document.createElement('div');
    toast.className = 'keyword-sync-toast';
    const list = names.slice(0, 6).map(escapeToastText).join(', ');
    const extra = names.length > 6 ? ` +${names.length - 6} more` : '';
    toast.innerHTML = `
      <div class="keyword-sync-toast__title">${escapeToastText(title)}</div>
  ...
  function selectedKeywordLabelsFromDom(trigger) {
    const explicit = trigger.getAttribute('data-keywords');
    if (explicit) {
      try {
    ...
    const selectors = [
      '[data-keyword-selected="true"]',
      '[data-selected-keyword]',
      '[data-keyword].is-selected',
      '[data-keyword].selected',
      '.keyword-chip.is-selected',
      '.keyword-chip.selected',
      '.chip.is-selected',
      '.chip.selected',
      '[aria-pressed="true"][data-keyword]'
    ];
    const root = trigger.closest('[data-keyword-selection-root]') || document;
    return Array.from(root.querySelectorAll(selectors.join(','))).map(el =>
      el.getAttribute('data-selected-keyword') ||
      el.getAttribute('data-keyword') ||
      el.textContent
    );
  }

  function bindAddSelectedToTreeDelegation() {
    document.addEventListener('click', (event) => {
      const trigger = event.target.closest('#addSelectedToTree,[data-add-selected-to-tree]');
      if (!trigger) return;
      const labels = selectedKeywordLabelsFromDom(trigger)
        .map(name => String(name || '').replace(/\s+/g, ' ').trim())
        .filter(Boolean);
      if (!labels.length) {
        showKeywordSyncToast('No selected keywords', []);
        return;
      }
      const result = window.ShopThatData.addKeywordsToGraph(labels, {
        parentName: trigger.getAttribute('data-tree-parent') || 'Patient Chart Keywords',
        source: trigger.getAttribute('data-keyword-source') || 'patient-chart'
      });
      const added = result.added || [];
      showKeywordSyncToast(added.length ? 'Keywords added to tree' : 'Already in tree', added.length ? added : labels);
    });
  }

  function initializeAfterStorageSync() {
    bootstrapStorageSync().finally(() => {
      window.ShopThatData.initialize();
      bindAddSelectedToTreeDelegation();
    });
  }
